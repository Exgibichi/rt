...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry)
{
    ...
    entry.push_back(Pair("time", (int64_t)wtx.tx->nTime));
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    {
        LOCK(cs_main);
        if (!IsV7Enabled(chainActive.Tip(), Params().GetConsensus()) && !GetBoolArg("-walletprematurewitness", false)) {
...
UniValue settxfee(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1 || AmountFromValue(request.params[0]) < (MIN_TX_FEE/10))
        throw runtime_error(
            "settxfee amount\n"
