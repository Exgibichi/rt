...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx,
...
bool AcceptToMemoryPoolWithTime(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx,
                        bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced,
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee)
{
    std::vector<uint256> vHashTxToUncache;
    bool res = AcceptToMemoryPoolWorker(pool, state, tx, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, vHashTxToUncache);
    if (!res) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx,
                        ...
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee)
{
    return AcceptToMemoryPoolWithTime(pool, state, tx, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee);
}
...
bool GetTransaction(const CDiskTxPos& postx, CTransactionRef &txOut)
{
    if (!fTxIndex)
        return false;
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (block.IsProofOfWork() && !CheckBlockProofOfWork(&block, consensusParams))
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("%s: invalid block=%s  height=%d  log2_trust=%.8g  date=%s  moneysupply=%s\n", __func__,
    ...
    CBlockIndex *tip = chainActive.Tip();
    assert (tip);
    LogPrintf("%s:  current best=%s  height=%d  log2_trust=%.8g  date=%s  moneysupply=%s\n", __func__,
...
bool DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean, bool fWriteNames)
{
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_trust=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utx) moneysupply=%s", __func__,
...
bool static DisconnectTip(CValidationState& state, const CChainParams& chainparams, bool fBare = false)
{
    ...
    if (!fBare) {
        ...
        for (const auto& it : block.vtx) {
            ...
            CValidationState stateDummy;
            if (tx.IsCoinBase() || tx.IsCoinStake() || !AcceptToMemoryPool(mempool, stateDummy, it, NULL, NULL, true)) {
    ...
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            if (fFailedChain || fMissingData) {
                ...
                if (fFailedChain && (pindexBestInvalid == NULL || pindexNew->nChainTrust > pindexBestInvalid->nChainTrust))
                    pindexBestInvalid = pindexNew;
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.GetBlockTime() > (int64_t)block.vtx[0]->nTime + nMaxClockDrift)
        return state.DoS(100, false, REJECT_INVALID, "bad-cb-time", false, "coinbase timestamp is too early");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSign)
{
    AssertLockHeld(cs_main);
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    LogPrintf("%s: hashBestChain=%s height=%d date=%s progress=%f trust=%s\n", __func__,
    ...
    try {
        ...
        while (num--) {
            ...
            if (nTime + nExpiryTimeout > nNow) {
                LOCK(cs_main);
                AcceptToMemoryPoolWithTime(mempool, state, tx, NULL, nTime);
                if (state.IsValid()) {
                    ++count;
                } else {
                    ++failed;
