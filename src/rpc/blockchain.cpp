...
UniValue gettxlistfor(const JSONRPCRequest& request)
{
    ...
    while (pblockindex->nHeight <= nToHeight)
    {
        ...
        map<uint256, CTransactionRef> mapFoundTx;
        ...
        for (const auto& tx : block.vtx)
        {
            ...
            for (const auto& txout : tx->vout)
            {
                ...
                for (const auto& addr : addresses) // 1 txout can contain more than 1 address
            ...
            if (found && (type == 0 || type == 1))
            {
                ...
                if (verbose)
                {
                    ...
                    for (const auto& txin : tx->vin)
                    {
                        ...
                        uint256 hashBlock;
                        if (!GetTransaction(txin.prevout.hash, prev, Params().GetConsensus(), hashBlock))
