...
UniValue generateBlocks(boost::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && !CheckBlockProofOfWork(pblock, Params().GetConsensus())) {
            ++pblock->nNonce;
            --nMaxTries;
        }
        if (nMaxTries == 0) {
...
UniValue getauxblock(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() == 0)
    {
            ...
            (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 20))
        {
            ...
            // Save
            mapNewBlock[pblock->GetHash()] = std::make_shared<CBlock>(*pblock);
        }
    ...
    else
    {
        ...
        std::shared_ptr<CBlock> pblock = mapNewBlock[hash];
        ...
        RegisterValidationInterface(&sc);
        bool fAccepted = ProcessNewBlock(Params(), pblock, true, NULL);
        UnregisterValidationInterface(&sc);
        if (fBlockPresent)
        {
            if (fAccepted && !sc.found)
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks     (numeric, does nothing - backward compatability with bitcoin RPC)\n"
            "\nResult:\n"
    ...
    return ValueFromAmount(MIN_TX_FEE / 10);
}
